...
export default function IrrigationSettings() {
  ...
  const solarModes: { key: SolarSubModeIrrigation; label: string; icon: string; desc: string }[] = [
    { key: 'BEFORE_SUNRISE', label: 'Before Sunrise', icon: '🌄', desc: 'Irrigation before sunrise' },
    { key: 'AFTER_SUNSET', label: 'After Sunset', icon: '🌇', desc: 'Irrigation after sunset' },
  ];
  ...
  const handleAdd = () => {
    ...
    setNewDeviceName('');
    setNewDevicePin('');
    Alert.alert("Success", "Irrigation device added and synchronized");
  };

  // ✅ Debug: Display state in console
  React.useEffect(() => {
    console.log('🔍 [IrrigationSettings] Current state:', state);
  }, [state]);
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={styles.statusCard}>
          ...
          <View style={styles.statusBadge}>
            <View style={[
              styles.statusDot, 
              { backgroundColor: state ? '#0EA5E9' : '#DC2626' }
            ]} />
            <Text style={[
              styles.statusValue, 
              { color: state ? '#0EA5E9' : '#DC2626' }
            ]}>
              {state ? 'IRRIGATION ACTIVE' : 'IRRIGATION STOPPED'}
            </Text>
          </View>
        ...
        {devices && devices.length > 0 ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>Add your first pump or solenoid valve</Text>
          </View>
        )}
